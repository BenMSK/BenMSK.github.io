...
*Given a vector space* **V** *over a field* **F** *of the real numbers* $$\mathbb{R}$$  *or complex numbers* **C**,<br/>
*a norm on* **V** *is a nonnegative-valued function* **$$f$$: V → R** *with the following properties:*<br/>
...
$$\lVert X \rVert_{2} \doteq \Sigma^N_{i}x^2_{i}$$ ($$x_i$$는 N차원의 벡터 X의 원소)
...
$$\lVert X \rVert_{1}\doteq \Sigma^N_{i} \mid x_{i}\mid  $$ ($$x_i$$는 N차원의 벡터 X의 원소)


---
## Supremum norm ($$\lVert\cdot \rVert_{\infty}$$)

$$\lVert X \rVert_{\infty}\doteq \max(\mid x_{1}\mid , \mid x_{2}\mid , \dots, \mid x_{N}\mid)$$ ($$x_i$$는 N차원의 벡터 X의 원소)
